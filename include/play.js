...
module.exports = {
  ...
  let thumb;
    if (song.thumbnail === undefined) thumb = "https://s3.gifyu.com/images/3dgifmaker63237.gif";
    else thumb = song.thumbnail.url;

  try {
      let embed = new MessageEmbed()
      .setColor("#F0EAD6")
      .setAuthor(`Started playing: ${song.title}`,'https://s3.gifyu.com/images/3dgifmaker63237.gif')
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        let nowPlaying = new MessageEmbed()
          .setAuthor('♪Now playing♪','https://s3.gifyu.com/images/3dgifmaker63237.gif','http://harmonymusic.tk')
      ...
      if (ms > 0 && ms<10000) {
        nowPlaying.addField("\u200b", "**``[" + createBar((ms == 0 ? seek : ms), seek, 25, "▬", "🔘")[0] + "]``**\n**" + "\n[" + new Date(seek * 1000).toISOString().substr(11, 8) + " / " + (ms == 0 ? " ◉ LIVE" : new Date(ms * 1000).toISOString().substr(11, 8))+ "]**" + "\n" + "\n **Time Remaining:**" + "``" + new Date(left * 1000).toISOString().substr(11, 8) + "``", false );
          ...
          .setAuthor("Searching...", "https://s3.gifyu.com/images/3dgifmaker63237.gif").setFooter("Lyrics")
          .setColor("#F0EAD6")
